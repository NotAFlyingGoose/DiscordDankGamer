    ...
    public void onMessage(BotMessageEvent event) {
        if (event.getArgs().length == 0) {
            EmbedBuilder embed = new EmbedBuilder()
                    ...
                    .setTimestamp(null);
            embed.addField("**To see all commands:**", "type `" + event.getBot().getPrefix() + "help page <number>`. replacing `<number>` with the page that you want to see.", false);
            embed.addField("**To get more info on a specific command:**", "type `" + event.getBot().getPrefix() + "help <command name>`. replacing `<command name>` with the command that you want to see.", false);
        ...
        if (event.getArgs()[0].equalsIgnoreCase("page")) {
            ...
            for (int i = 25 * (page - 1); i < commands.size() && i != 25 * page; i++) {
                ...
                embed.addField(
                        "🎟️ - " + builder.name() + " - 🎟️",
                        "**- Usages:** " + Util.codeArrayToString(CommandManager.getUsages(event.getBot().getPrefix(), builder)), true);
            }
            embed.setFooter("to see more info on a specific command do `" + event.getBot().getPrefix() + "help <command name>`");
            event.getChannel().sendMessage(embed.build()).queue();
        } else {
            List<Command> commands = event.getCommandManager().getCommands();
            for (Command command : commands) {
                ...
                if (builder.name().equals(event.getArg(0)) || Arrays.asList(builder.aliases()).contains(event.getArg(0))) {
                    ...
                    commandData+="**Usages :** " + Util.codeArrayToString(CommandManager.getUsages(event.getBot().getPrefix(), builder)) + " \n";
